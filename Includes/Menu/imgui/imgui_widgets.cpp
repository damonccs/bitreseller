...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 bg_col_hover = GetColorU32(ImGuiCol_ButtonHovered);
    for (int i = 1; i < 10; i++) {
    ...
    if (*v)
    {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 + 10, check_bb.Min.y + 9), 10, bg_col, 12);
    }
    else
    {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2 - 10, check_bb.Min.y + 9), 10, bg_col_hover, 12);
    }
    if (label_size.x > 0.0f)
...
bool ImGui::Checkbox2(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(150, 0) + ImVec2(square_sz + square_sz, label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
        return false;
    ...
    if (*v)
        it_anim->second.active_text_anim += 2 * (1.f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.active_text_anim -= 2 * (1.f - ImGui::GetIO().DeltaTime);
    ...
    it_nameanim->second = ImClamp(it_nameanim->second + (5.f * ImGui::GetIO().DeltaTime * (*v ? 1.f : -1.0f)), 0.f, 1.0f);
    it_nameanim->second *= style.Alpha;

    //window->DrawList->AddRectFilled(check_bb.Min + ImVec2(3 + 280, 7), check_bb.Max + ImVec2(13 + 280, -10), GetColorU32(ImGuiCol_FrameBg), 10);

    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(5 + 140, 9), check_bb.Max + ImVec2(11 + 140, -12), ImColor(0.04, 0.04, 0.04, 1.00f - it_nameanim->second), 10);

    window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(5 + 140, 9), check_bb.Max + ImVec2(it_nameanim->second * 11 + 135, -12),
        ImColor(accent_color[0] / 6, accent_color[1] / 6, accent_color[2] / 6),
        ImColor(accent_color[0], accent_color[1], accent_color[2]),
        ImColor(accent_color[0], accent_color[1], accent_color[2]),
        ImColor(accent_color[0] / 6, accent_color[1] / 6, accent_color[2] / 6));

    window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + 140 + 13 + it_nameanim->second * 18, check_bb.Min.y + 12), 10.f, ImColor(34 / 255.f, 37 / 255.f, 43 / 255.f, 1.00f - it_nameanim->second), 24);

    //window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + 100 + 13 + it_nameanim->second * 18, check_bb.Min.y + 12), 10.f, ImColor(accent_color[0], accent_color[1], accent_color[2], it_nameanim->second), 24);
    ImVec2 center(check_bb.Min.x + 140 + 13 + it_nameanim->second * 18, check_bb.Min.y + 12);
    float radius = 10.f;
    int numSegments = 24;

    ImColor color1(accent_color[0], accent_color[1], accent_color[2], it_nameanim->second);
    ImColor color2(accent_color[0] / 2, accent_color[1] / 2, accent_color[2] / 2, it_nameanim->second);
    ImColor color3(accent_color[0], accent_color[1], accent_color[2], it_nameanim->second * 0.5f);

    window->DrawList->AddCircleFilled(center, radius, color1, numSegments);
    window->DrawList->AddCircleFilled(center, radius - 2.f, color2, numSegments);
    window->DrawList->AddCircleFilled(center, radius - 4.f, color3, numSegments);
    ...
    if (label_size.x > 0.0f)
    {
        if (*v)
        {
            ...
            window->DrawList->AddText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x - 30, check_bb.Min.y + style.FramePadding.y - 0.8f), ImColor(255, 255, 255, (155 + it_anim->second.active_text_anim)), label);
            ImGui::PopStyleColor(1);
        }
        else
        {
            //ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 0.5f));
            window->DrawList->AddText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x - 30, check_bb.Min.y + style.FramePadding.y - 0.8f), ImColor(148, 163, 184, (155 + it_anim->second.active_text_anim)), label);
            // ImGui::PopStyleColor();
        }
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, - 7.f), window->DC.CursorPos + ImVec2(w, label_size.y + 7.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    RenderFrame({ frame_bb.Min.x, frame_bb.Min.y + 8.f }, { grab_bb.Max.x, frame_bb.Max.y - 8.f }, ImU32(ImColor(75, 83, 32, 255)), true, g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min + ImVec2(0, 5.f), { grab_bb.Max.x - 9.f, grab_bb.Max.y - 5.f }, ImU32(ImColor(104, 117, 36, 255)), 2.f);
    ...
    RenderTextClipped({ grab_bb.Min.x + 6.f, frame_bb.Min.y }, { grab_bb.Max.x + 33.f,  frame_bb.Max.y }, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::InputTextCPP(const char* label, std::string* str, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size + 20, ImVec2(420, 40), flags, callback, user_data);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImGui::SetCursorPosX(CalcItemWidth() - 295);

    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 20.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min + ImVec2(0, 0), frame_bb.Max, GetColorU32(ImGuiCol_Header), true, style.FrameRounding);
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 1), ImVec2(total_bb.Min.x + 419, total_bb.Min.y + 39), ImColor(20, 20, 20), style.FrameRounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding + ImVec2(0.0f, 6.f);
    //ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding;
    ImVec2 text_size(0.0f, 0.0f);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize), rect_pos + ImVec2(rect_size.x, bg_offy_dn));
                    rect.ClipWith(clip_rect);
                    if (rect.Overlaps(clip_rect)) draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(1, -1), rect.Max + ImVec2(1, -2), ImColor(40, 40, 40), 5);//fucking select backround
                }
                rect_pos.x = draw_pos.x - draw_scroll.x;
                rect_pos.y += g.FontSize;
            }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_filing->second.value + 0, 11), frame_bb.Min + ImVec2(it_filing->second.value + 1, 30), GetColorU32(ImGuiCol_Text));//fucking sellect txt
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - CalcItemWidth() - 41, frame_bb.Min.y + style.FramePadding.y), label);
